...
class EmgAnnotationPrediction:
    ...
    def __init__(self, model_weights_name_anno, model_weights_name_MUAP, annotation_path, destination_path, batch_size=32, loudness_target=-26.0,
                 ...
                 sliding_window_size=0.1, sample_time=2, base_path='H:/Downloads/Stage/',
        ...
        self.model_weights_path_anno = self.base_path + model_weights_name_anno
        self.model_weights_path_MUAP = self.base_path + model_weights_name_MUAP
    ...
    def _predict_labels(self):
        ...
        for filename in tqdm(self.annotated_files):
            ...
            predicted_labels, perc_breed, perc_smal, perc_normaal, true_label, tabellabel = self.generate_batched_predictions(data, base_name)
            print(tabellabel)
            if tabellabel == 0 :
    ...
    def generate_batched_predictions(self, data, base_name):
        ...
        for j in range(0, int(np.ceil(len(data_iterations) / self.batch_size))):
            ...
            for i in range(j * self.batch_size, (j + 1) * self.batch_size):
                ...
                if predictions[0][0]>0.75: #rest
                    ...
                    signal_type = 0
                '''
                if predictions[0][0]>predictions[0][1] and predictions[0][0]>predictions[0][2]: #rest
                    signal_type = 1
                elif predictions[0][1]>predictions[0][0] and predictions[0][1]>predictions[0][2]: #contraction
                    signal_type = 2
                elif predictions[0][2]>predictions[0][0] and predictions[0][2]>predictions[0][1]: #needle
                    signal_type = 3
                else:
                    signal_type = 0
        ...
        # to zero inputs.
            ...
            if base_name in self.breed:
                ...
                print('No true label')
                tabellabel=7
            return predicted_labels, perc_breed, perc_smal, perc_normaal, true_label, tabellabel
        else: 
            return 0, 0, 0, 0, 0, 0
...
if __name__ == '__main__':
    input_dictionary = {
        'model_weights_name_signal_type': 'test.hdf5',
        'model_weights_name_MUAP': 'hard_label_models/MUAPfinalsaveset_weights.2-11-0.815962_0.611400.hdf5',
        'annotation_path': 'test_evaluation/',
        'destination_path': 'Predicted_labels/'
    }
    emg_annotation_prediction = EmgAnnotationPrediction('test.hdf5','hard_label_models/MUAPfinalsaveset_weights.2-11-0.815962_0.611400.hdf5','test_evaluation/','Predicted_labels/')
