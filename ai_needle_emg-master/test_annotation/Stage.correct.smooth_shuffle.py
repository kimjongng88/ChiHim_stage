...
This file trains with looking at the amount of reviewers, experience and label distribution. This is part of project of Chi Him
"""
import numpy as np
...
def train_model(model,d,c,batch_size,INIT_LR):
    ...
    for train_index, test_index in skf.split(d,c):
        ...
        print(d[1].shape,'shape nodig')

        print(len(trainX),len(trainY),len(testX),len(testY))
        print(type(trainX),'type')
        print(trainY.shape,trainX.dtype)
        ...
        model.save('model_n'+ str(fold_no)+ '.hdf5')
        print('model gesaved')
...
if __name__ == '__main__':
    ...
    trainX= np.load('data_s_02.npy',mmap_mode='r',allow_pickle=True)
    trainX_sub = np.load('data_s_03.npy',mmap_mode='r',allow_pickle=True)
    print(len(trainX),len(trainX_sub))
    trainX = da.concatenate([trainX,trainX_sub],axis=0)
    del trainX_sub
    trainX_sub = np.load('data_s_04.npy',mmap_mode='r',allow_pickle=True)
    trainX = da.concatenate([trainX,trainX_sub],axis=0)
    print(len(trainX),len(trainX_sub))
    del trainX_sub
    trainX_sub = np.load('data_s_05.npy',mmap_mode='r',allow_pickle=True)
    trainX = da.concatenate([trainX,trainX_sub],axis=0)
    print(len(trainX),len(trainX_sub))
    del trainX_sub
    trainX_sub = np.load('data_s_06.npy',mmap_mode='r',allow_pickle=True)
    trainX = da.concatenate([trainX,trainX_sub],axis=0)
    print(len(trainX),len(trainX_sub))
    del trainX_sub
    trainX_sub = np.load('data_s_07.npy',mmap_mode='r',allow_pickle=True)
    trainX = da.concatenate([trainX,trainX_sub],axis=0)
    print(len(trainX),len(trainX_sub))
    del trainX_sub
    trainX_sub = np.load('data_s_08.npy',mmap_mode='r',allow_pickle=True)
    trainX = da.concatenate([trainX,trainX_sub],axis=0)
    print(len(trainX),len(trainX_sub))
    trainX_sub = np.load('data_s_085.npy',mmap_mode='r',allow_pickle=True)
    trainX = da.concatenate([trainX,trainX_sub],axis=0)
    print(len(trainX),len(trainX_sub))
    trainX_sub = np.load('data_s_09.npy',mmap_mode='r',allow_pickle=True)
    trainX = da.concatenate([trainX,trainX_sub],axis=0)
    print(len(trainX),len(trainX_sub))
    del trainX_sub
    trainX_sub = np.load('data_s_c1.npy',mmap_mode='r',allow_pickle=True)
    trainX = da.concatenate([trainX,trainX_sub],axis=0)
    print(len(trainX),len(trainX_sub))
    del trainX_sub
    trainX_sub = np.load('data_s_r.npy',mmap_mode='r',allow_pickle=True)
    trainX = da.concatenate([trainX,trainX_sub],axis=0)
    print(len(trainX),len(trainX_sub))
    del trainX_sub
    trainX_sub = np.load('data_s_n.npy',mmap_mode='r',allow_pickle=True)
    trainX = da.concatenate([trainX,trainX_sub],axis=0)
    ...
    trainY= np.load('datay_s_02.npy',mmap_mode='r',allow_pickle=True)
    trainY_sub = np.load('datay_s_03.npy',mmap_mode='r',allow_pickle=True)
    print(len(trainY),len(trainY_sub))
    trainY = da.concatenate([trainY,trainY_sub],axis=0)
    del trainY_sub
    trainY_sub = np.load('datay_s_04.npy',mmap_mode='r',allow_pickle=True)
    trainY = da.concatenate([trainY,trainY_sub],axis=0)
    print(len(trainY),len(trainY_sub))
    del trainY_sub
    trainY_sub = np.load('datay_s_05.npy',mmap_mode='r',allow_pickle=True)
    trainY = da.concatenate([trainY,trainY_sub],axis=0)
    print(len(trainY),len(trainY_sub))
    del trainY_sub
    trainY_sub = np.load('datay_s_06.npy',mmap_mode='r',allow_pickle=True)
    trainY = da.concatenate([trainY,trainY_sub],axis=0)
    print(len(trainY),len(trainY_sub))
    del trainY_sub
    trainY_sub = np.load('datay_s_07.npy',mmap_mode='r',allow_pickle=True)
    trainY = da.concatenate([trainY,trainY_sub],axis=0)
    print(len(trainY),len(trainY_sub))
    del trainY_sub
    trainY_sub = np.load('datay_s_08.npy',mmap_mode='r',allow_pickle=True)
    trainY = da.concatenate([trainY,trainY_sub],axis=0)
    print(len(trainY),len(trainY_sub))
    del trainY_sub
    trainY_sub = np.load('datay_s_085.npy',mmap_mode='r',allow_pickle=True)
    trainY = da.concatenate([trainY,trainY_sub],axis=0)
    print(len(trainY),len(trainY_sub))
    del trainY_sub
    trainY_sub = np.load('datay_s_09.npy',mmap_mode='r',allow_pickle=True)
    trainY = da.concatenate([trainY,trainY_sub],axis=0)
    print(len(trainY),len(trainY_sub))
    del trainY_sub
    trainY_sub = np.load('datay_s_c1.npy',mmap_mode='r',allow_pickle=True)
    trainY = da.concatenate([trainY,trainY_sub],axis=0)
    print(len(trainY),len(trainY_sub))
    del trainY_sub
    trainY_sub = np.load('datay_s_r.npy',mmap_mode='r',allow_pickle=True)
    trainY = da.concatenate([trainY,trainY_sub],axis=0)
    print(len(trainY),len(trainY_sub))
    del trainY_sub
    trainY_sub = np.load('datay_s_n.npy',mmap_mode='r',allow_pickle=True)
    trainY = da.concatenate([trainY,trainY_sub],axis=0)
    print(len(trainY),len(trainY_sub))
    del trainY_sub
    print(len(trainY))
