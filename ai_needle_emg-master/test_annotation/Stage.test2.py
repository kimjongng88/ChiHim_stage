...
def dense_net_training_generator(train_files,full_label_list,augmentation_flag=True):
        ...
        for f, element in train_files:
            ...
            if element['name'] not in A:
                ...
                pass
            a += 1
        print('begin array vormen',len(X),len(Y))
        Y = np.array(Y,dtype=np.float16)
        X = np.array(X,dtype=np.float16)
        print(X.shape,Y.shape)
        np.save("data_s_r.npy",X)
        np.save("datay_s_r.npy",Y)
        print('convert done')
        print('gesaved')
        return X,Y
...
def begin(param):
    ...
    learning_rate = param['learning_rate']#INIT_LR
    base_layers_trainable_flag = False
    layers = param['layers']


    print(batch_size,learning_rate,layers)

    
    #trainX = parse_npy('losse npy/0/Rest1/*.npy')
    
    
    #trainX = parse_npy('losse npy/*/*/*.npy')
    #print(type(trainX))
    #save('data.npy', trainX)
    #print('done saving')
    
    #data2 = np.load('data2.npy',allow_pickle=True)
    #trainX = np.load('ai_needle_emg-master/test_annotation/npy_array_pre/data_0_contraction.npy',allow_pickle=True)
    #print(len(data),len(data2))
    #res = np.concatenate((data,data2))


    #save('data_test.npy', res)
    #print(type(data2))
    #print(data2,len(data2))



    #trainX, valX = parse_npy('ai_needle_emg-master/test_annotation/test_npy/0.5/needle/*.npy','ai_needle_emg-master/test_annotation/test_npy/0.5/contraction/*.npy', 'ai_needle_emg-master/test_annotation/test_npy/0.5/rest/*.npy')
    #full_label_list = get_list('ai_needle_emg-master/test_annotation/test_list/test_model_list.txt')
    #print(type(res))
    #trainX, valX= parse_npy('Experimentation/Experiments/*/Needle/*.npy','Experimentation/Experiments/*/Contraction/*.npy', 'Experimentation/Experiments/*/Rest/*.npy')
    #full_label_list = get_list('ai_needle_emg-master/analyse/smoothed_labels/all_smoothed.txt') # VOOR ALLE GESOORTEERDE
    #full_label_list = get_list('ai_needle_emg-master/analyse/smoothing/full_smoothed.txt') 
    #testX, testY = dense_net_training_generator(testX,full_label_list)
    #print(type(full_label_list))
    #trainX, trainY = dense_net_training_generator(trainX,full_label_list)

    trainX= np.load('data_s_n.npy',mmap_mode='r',allow_pickle=True)
    trainY= np.load('datay_s_n.npy',mmap_mode='r',allow_pickle=True)

    model = build_model(layers,True)
    validation_loss, model = train_model(model,trainX,trainY,batch_size,learning_rate,param)
